...
def main():
    ...
    query.grad.zero_()
    del selected_blocks,sparse_mask,ptm_attn_result,ptm_attn_result_sum
    torch.cuda.empty_cache()
    torch.cuda.reset_peak_memory_stats()

    fa2_result = flash_attn(query,key,value)
    fa2_fwd_memory = torch.cuda.max_memory_allocated()
    print(f'fa2 fwd memory:{fa2_fwd_memory/1024**2} MB')
    fa2_result_sum = fa2_result.sum()*fa2_result.sum()
    torch.cuda.reset_peak_memory_stats()
    fa2_result_sum.backward(retain_graph=True)
    fa2_bwd_memory = torch.cuda.max_memory_allocated()
    print(f'fa2 bwd memory:{fa2_bwd_memory/1024**2} MB')
    query.grad.zero_()
    del fa2_result,fa2_result_sum
    torch.cuda.empty_cache()
    torch.cuda.reset_peak_memory_stats()

    fa3_result = flash_attn3(query,key,value)
    fa3_fwd_memory = torch.cuda.max_memory_allocated()
    print(f'fa3 fwd memory:{fa3_fwd_memory/1024**2} MB')
    fa3_result_sum = fa3_result.sum()*fa3_result.sum()
    torch.cuda.reset_peak_memory_stats()
    fa3_result_sum.backward(retain_graph=True)
    fa3_bwd_memory = torch.cuda.max_memory_allocated()
    print(f'fa3 bwd memory:{fa3_bwd_memory/1024**2} MB')
    query.grad.zero_()
    del fa3_result,fa3_result_sum
    torch.cuda.empty_cache()
    torch.cuda.reset_peak_memory_stats()
    ...
    experiment_data['fa2_fwd_memory'] = fa2_fwd_memory
    experiment_data['fa2_bwd_memory'] = fa2_bwd_memory
    experiment_data['fa3_fwd_memory'] = fa3_fwd_memory
    experiment_data['fa3_bwd_memory'] = fa3_bwd_memory
    experiment_data['sparge_fwd_memory'] = sparge_fwd_memory

    # 使用方法1
    if args.save == 'true':
        append_to_csv(experiment_data,f'../results/full_memory_results_{block_size}.csv')
