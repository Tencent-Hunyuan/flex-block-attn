...
def main():
    ...
    window_size = int((q.size(2) // block_size) * sparse_rate)
    print(f"window_size:{window_size}")
    selected_blocks = generate_swa_selected_blocks(q, block_size, window_size)
    # print(f'selected_blocks:{selected_blocks}')

    create_torch_mask_time = do_bench(
        fn=lambda: create_torch_mask(q, block_size, selected_blocks),
        warmup=2,
        rep=8,
        return_mode="mean",
    )
    # create_torch_mask_time = do_bench(fn=lambda:create_torch_swa_mask(q,block_size,window_size),warmup=2,rep=8,return_mode="mean")
    # create_torch_mask_time = 0
    create_sparse_mask_time = do_bench(
        fn=lambda: create_sparse_mask(q, block_size, selected_blocks),
        warmup=2,
        rep=8,
        return_mode="mean",
    )
    # 由于是测试full的情况
    create_flex_mask_time = do_bench(
        fn=lambda: create_flex_swa_mask(q, block_size, window_size),
        warmup=2,
        rep=8,
        return_mode="mean",
    )
    ...
    # print(f'torch_mask:{torch_mask}')
    # torch_mask = create_torch_swa_mask(q,block_size,window_size)
    # torch_mask = None
    sparse_mask = create_sparse_mask(q, block_size, selected_blocks)
    # print(f'sparse_mask:{sparse_mask}')

    flex_mask = create_flex_swa_mask(q, block_size, window_size)
    # flex_mask = None
    # print(f'flex_mask:{flex_mask}')

    torch_flex_execute_time = do_bench(
        fn=lambda: torch_flex_attn(q, k, v, flex_mask),
        warmup=2,
        rep=8,
        return_mode="mean",
    )
    ...
    torch_attn_execute_time = do_bench(
        fn=lambda: torch_attn(q, k, v, torch_mask), warmup=2, rep=8, return_mode="mean"
    )
    ...
    magi_swa_mask_time = do_bench(
        fn=lambda: magi_swa_getqkranges(query, key, value, block_size, window_size),
        warmup=2,
        rep=8,
        return_mode="mean",
    )
    q_ranges, k_ranges, attn_type_map = magi_swa_getqkranges(
        query, key, value, block_size, window_size
    )

    magi_attn_execute_time = do_bench(
        fn=lambda: magi_attn(query, key, value, q_ranges, k_ranges, attn_type_map),
        warmup=2,
        rep=8,
        return_mode="mean",
    )
    ...
    sparge_swa_mask_time = do_bench(
        fn=lambda: create_sparge_swa_mask(q, block_size, 64, window_size),
        warmup=2,
        rep=8,
        return_mode="mean",
    )
    mask_id = create_sparge_swa_mask(q, block_size, 64, window_size)
    sparge_attn_execute_time = do_bench(
        fn=lambda: block_sparse_sage2_attn_cuda(
            q,
            k,
            v,
            mask_id=mask_id,
    ...
    ptm_attn_execute_time = do_bench(
        fn=lambda: ptm_sparse_attn(q, k, v, block_size, sparse_mask),
        warmup=2,
        rep=8,
        return_mode="mean",
    )
    ...
    result_torch_magi = torch.allclose(
        torch_attn_result, magi_attn_result, rtol=5e-3, atol=5e-3
    )
    result_ptm_torch = torch.allclose(
        ptm_attn_result, torch_attn_result, rtol=5e-3, atol=5e-3
    )
    result_torch_torch = torch.allclose(
        torch_attn_result, torch_attn_result, rtol=5e-3, atol=5e-3
    )
    result_torch_flex = torch.allclose(
        torch_attn_result, flex_attn_result, rtol=5e-3, atol=5e-3
    )
    ...
    result_torch_sparge = torch.allclose(
        torch_attn_result, sparge_attn_result, rtol=5e-3, atol=5e-3
    )
    ...
    print(f"create_magi_mask_time:{magi_swa_mask_time}")
    ...
    experiment_data["magi_generate_masks"] = magi_swa_mask_time
    ...
    # 使用方法1
    if args.save == "true":
        append_to_csv(experiment_data, f"../results/swa_fwd_results_{block_size}.csv")
