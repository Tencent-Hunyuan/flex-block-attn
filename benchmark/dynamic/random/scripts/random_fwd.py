...
def main():
    ...
    # 由于是测试full的情况
    # create_flex_mask_time = do_bench(fn=lambda:create_flex_swa_mask(q,block_size,window_size),warmup=2,rep=8,return_mode="mean")
    create_flex_mask_time = do_bench(
        fn=lambda: create_flex_block_mask(q, torch_mask),
        warmup=2,
        rep=8,
        return_mode="mean",
    )
    ...
    magi_get_qkranges_time = do_bench(
        fn=lambda: magi_get_qkranges(query, key, value, block_size, selected_blocks),
        warmup=2,
        rep=8,
        return_mode="mean",
    )
    ...
    if args.save == "true":
        append_to_csv(
            experiment_data, f"../results/random_fwd_results_{block_size}.csv"
        )
